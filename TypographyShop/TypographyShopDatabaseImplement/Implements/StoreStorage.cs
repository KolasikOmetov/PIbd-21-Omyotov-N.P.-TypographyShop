...
namespace TypographyShopDatabaseImplement.Implements
{
    public class StoreStorage : IStoreStorage
    {
        ...
        private Store CreateModel(StoreBindingModel model, Store store, TypographyShopDatabase context)
        {
        ...
        public void Extract(int PrintedId, int Count)
        {
            using (var context = new TypographyShopDatabase())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        foreach (var pc in context.PrintedComponents.Where(rec => rec.PrintedId == PrintedId))
                        {
