...
namespace TypographyShopListImplement.Implements
{
	public class PrintedStorage : IPrintedStorage
	{
		...
		{
			Printed tempPrinted = new Printed { Id = 1, PrintedComponents = new Dictionary<int, int>() };
			foreach (var Printed in source.Printeds)
			{
		...
		private PrintedViewModel CreateModel(Printed Printed)
		{
			...
			Dictionary<int, (string, int)> PrintedComponents = new Dictionary<int, (string, int)>();
